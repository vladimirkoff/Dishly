...
final class PrepareViewController: UIViewController, Storyboardable {
    ...
    var viewModel: PrepareVMProtocol!
    ...
    func saveRecipe() {
        guard let viewModel = viewModel, let recipe = recipeViewModel else { return }
        showLoader(true)
        viewModel.createRecipe(recipe: recipe, image: recipeImage) { [weak self] error in
